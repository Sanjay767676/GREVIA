...
// Technician workflow actions driven by current status.
export function WorkerActions({
  ...
  const acceptableStatuses: ComplaintStatus[] = [
    STATUS.ASSIGNED,
    STATUS.REOPENED,
    ...
    <div className="space-y-3">
      ...
      {canResolve && (
        <div className="space-y-2 rounded-lg border border-slate-200 p-3">
          ...
          />
          <label className="label">Resolution proof (optional)</label>
          <input
            type="file"
            accept="image/*"
      ...
      <div className="flex flex-wrap gap-3">
        {canAccept && (
          <button onClick={() => act('ACCEPT')} disabled={loading} className="btn-primary">
            Accept
          </button>
        )}
        {canStart && (
          <button onClick={() => act('START')} disabled={loading} className="btn-secondary">
            Start work
          </button>
        )}
        {canResolve && (
